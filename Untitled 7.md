...
|#|Vấn đề|Mức độ|
|---|---|---|
|1|**Agent-per-Operation Anti-Pattern**|🔴 Critical|
||11 agents cho công việc của 5 agents (MathSum/Sub, 4 YouTube agents riêng biệt)||
|2|**Race Condition**|🔴 Critical|
||Nhiều agents subscribe cùng topic → wasted processing||
|3|**Thiếu Correlation ID**|🔴 Critical|
||Không trace được request qua các agents||
|4|**Synthesizer không biết chờ bao nhiêu results**|🔴 Critical|
||Generate report ngay khi nhận 1 result, có thể mất data||
|5|**Không có Error Propagation**|🔴 Critical|
||Agent crash → các agents khác chờ mãi mãi||
|6|**Naming không chuẩn**|🔴 Critical|
||"Chief", "Pool" không phải industry standard||
|7|**Dead Code**|🟡 Medium|
||ChiefAgent, RouterAgent tồn tại nhưng không dùng||

### ✅ 6 Điểm Tốt

1. **Base Agent Design** - Clear abstraction, async-first
2. **MessageBus** - Redis PubSub + fallback + deduplication
3. **PlanStep Model** - Well-structured dataclass
4. **AgentToolCatalog** - Extensible registry
5. **Circuit Breaker** - Có sẵn (chưa integrate)
6. **Layered Architecture** - layer_0/1/2/3 rõ ràng

### 📈 Đề Xuất Refactor

```
TRƯỚC (11 agents):                    SAU (5-6 agents):
├── MathAgentSum                      ├── MathAgent (merge 2→1)
├── MathAgentSubtract                 ├── DateAgent
├── DateAgentToday                    ├── YouTubeAgent (merge 4→1)
├── YouTubeAgentOpen                  ├── PlannerAgent
├── YouTubeAgentSearch                ├── SynthesizerAgent
├── YouTubeAgentPlay                  └── Orchestrator
├── YouTubeAgentInfo
├── PlannerAgent
├── SynthesizerAgent
├── ChiefAgent (không dùng)
└── RouterAgent (không dùng)
```

### ⏱️ Thời Gian Ước Tính: 5-7 ngày

| Priority | Task                      | Effort   |
| -------- | ------------------------- | -------- |
| P1       | Merge agents (11→5)       | 2-3 days |
| P1       | Add correlation ID        | 1 day    |
| P2       | Add execution_started     | 1 day    |
| P3       | Remove dead code          | 0.5 day  |
| P3       | Integrate circuit breaker | 1-2 days |
