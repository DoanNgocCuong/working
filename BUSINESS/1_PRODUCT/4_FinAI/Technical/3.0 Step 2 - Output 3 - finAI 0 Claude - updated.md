# FinAI Multi-Agent Architecture (Sử dụng kiến trúc Q4 để về sau dễ mở rộng. Vì Q4 khi bật tuyến tính thì sẽ giống Q3)
...
- Không micromanage, chỉ điều phối
...
- Nhưng bây giờ là agent độc lập với LLM reasoning

---

### 2.3. Navigator Agent (Điều khiển trình duyệt)
...
│   └── models.py
│
├── layer_2_cognition/           # ⭐ THAY ĐỔI HOÀN TOÀN
│   ├── __init__.py
│   │
...
|Aspect|ReAct Pattern (Cũ)|Multi-Agent (Mới)|
|---|---|---|
...
- Nếu có issues, rollback về ReAct pattern
- Đã có code cũ làm reference
    ...
    async def execute_action(self, action: str, msg: Message):
        ...
        if action == "return_to_user":
            ...
            if not self._report_received:
                ...
                logger.warning(f"[{self.name}] ⚠️ Already returned final report, skipping")
    ...
    async def decide_next_action(self, msg: Message) -> str:
        ...
        if msg.topic == "task_available":
            ...
            if self.retry_count >= self.max_retries:
                logger.warning(f"[{self.name}] ⚠️ Max retries ({self.max_retries}) reached")
                return "STOP"
            return "adjust_plan"
...
class NavigatorAgent(P2PAgent):
    ...
    async def _execute_navigation_plan(self, msg: Message):
        ...
        if not self.current_plan:
            logger.warning(f"[{self.name}] ⚠️ No navigation plan provided")
            return
        ...
        for step_idx, step in enumerate(self.current_plan):
            ...
            if not url:
                logger.warning(f"[{self.name}] ⚠️ Step {step_idx + 1} missing URL, skipping")
                continue
...
class VerifierAgent(P2PAgent):
    ...
    async def _verify_extracted_data(self, msg: Message):
        ...
        if needs_renavigation:
            logger.warning(f"[{self.name}] ⚠️ Data insufficient, requesting re-navigation")
...
| Benefit | Description |
|---------|-------------|
| **Standardization** | Giao thức chuẩn, dễ integrate với nhiều AI clients |
| **Security** | Built-in security model với permission controls |
| **Discoverability** | AI clients tự động discover available tools |
