# 1. ...
## 1.1  tạo bảng: studio_bot_workflow trong postgres

```

```

## 1.2 Xoá toàn bộ dữ liệu của bảng 
```
TRUNCATE TABLE studio_bot_workflow;
```

`TRUNCATE TABLE studio_bot_workflow RESTART IDENTITY;`


- `TRUNCATE TABLE ...;` → id **không** về lại 1.
    
- `TRUNCATE TABLE ... RESTART IDENTITY;` → id **sẽ** về lại 1 (nếu sequence không có giá trị start khác).

### Khi tăng ID bị vấn đề 

1. Các lệnh để kiểm tra vấn đề  
2. Chỉ ra vấn đề  
3. Fix

```
SELECT id FROM studio_bot_workflow ORDER BY id DESC LIMIT 1;  
  
SELECT last_value FROM studio_bot_workflow_id_seq;
```

```
- Trong bảng `studio_bot_workflow` **ID cao nhất = 10011**
    
- Nhưng sequence `studio_bot_workflow_id_seq` **đang ở số 5**
    

➡️ **Sequence đang chạy sai hoàn toàn** → backend insert tiếp → nó sẽ cố gán ID = 6 → nhưng ID = 6 đã tồn tại → **trùng khóa → 500**.
```

### Tăng id tự động

```
SELECT setval('studio_bot_workflow_id_seq', (SELECT MAX(id) FROM studio_bot_workflow));
```

```
# FIX DỨT ĐIỂM (bắt buộc phải làm)

Chạy lệnh này trong PostgreSQL:

`SELECT setval('studio_bot_workflow_id_seq', (SELECT MAX(id) FROM studio_bot_workflow));`

Bạn sẽ thấy sequence nhảy lên:

`last_value = 10011`

Sau đó backend insert sẽ sinh tiếp:

`10012, 10013, 10014, ...`

➡️ **Không còn báo lỗi trùng ID nữa.**
```

```
# ❗ Tại sao sequence tụt về 5?

Có 3 lý do phổ biến:

### 1. Bạn đã import/restore DB thủ công

Sequence không restore theo.

### 2. Bạn đã TRUNCATE nhưng không dùng RESTART

→ Sequence bị reset sai.

### 3. Bạn đã ALTER/RENAME TABLE

→ Sequence bị detached.

### 4. Bạn khi đổi schema JSONB → TEXT

Có thể đã tạo lại table (hoặc drop/restore table) không reset sequence chuẩn.
```

## 1.3 Cách lấy lại history của lesson_... trong postgres

### ✅ **1. Bảng MySQL bạn cung cấp**

|Tên cột|MySQL Type|Ghi chú|
|---|---|---|
|id|int, auto_increment|PRIMARY KEY|
|name|varchar(128)||
|description|varchar(2048)||
|scenario|json||
|create_at|bigint||
|update_at|bigint||
|is_loading|tinyint(1)|0/1|

---

### 🟦 **2. Tương đương trong PostgreSQL**

| MySQL            | PostgreSQL                            | Giải thích                      |
| ---------------- | ------------------------------------- | ------------------------------- |
| `int`            | `INTEGER`                             | giống nhau                      |
| `auto_increment` | `SERIAL` hoặc `GENERATED AS IDENTITY` | auto tăng                       |
| `varchar(128)`   | `VARCHAR(128)`                        | giống nhau                      |
| `varchar(2048)`  | `VARCHAR(2048)`                       | giống nhau                      |
| `json`           | `JSONB` (khuyên dùng) hoặc `JSON`     | JSONB nhanh hơn                 |
| `bigint`         | `BIGINT`                              | giống                           |
| `tinyint(1)`     | **BOOLEAN** (0/1) hoặc `SMALLINT`     | PostgreSQL **không có tinyint** |


```
CREATE TABLE lesson_studio_bot (
    id SERIAL PRIMARY KEY,
    name VARCHAR(128),
    description VARCHAR(2048),
    scenario JSONB,
    create_at BIGINT,
    update_at BIGINT,
    is_loading BOOLEAN DEFAULT FALSE
);
```

```
ALTER TABLE lesson_studio_bot ADD COLUMN system_prompt TEXT;
```
```
ALTER TABLE lesson_studio_bot 
    ADD COLUMN IF NOT EXISTS system_prompt TEXT,
    ADD COLUMN IF NOT EXISTS provider_name VARCHAR(255),
    ADD COLUMN IF NOT EXISTS generation_params JSONB,
    ADD COLUMN IF NOT EXISTS system_extraction_variables JSONB,
    ADD COLUMN IF NOT EXISTS system_prompt_generation TEXT,
    ADD COLUMN IF NOT EXISTS system_extraction_profile JSONB,
    ADD COLUMN IF NOT EXISTS format_output TEXT,
    ADD COLUMN IF NOT EXISTS data_excel JSONB;
```

```
  
ALTER TABLE lesson_studio_bot  
    ALTER COLUMN system_extraction_variables TYPE TEXT;  
  
ALTER TABLE lesson_studio_bot  
    ALTER COLUMN system_extraction_profile TYPE TEXT;
```
---
## 1.4 # Lấy toàn bộ cột và bảng trong đây 

```
SELECT 
    table_name, 
    column_name
FROM 
    information_schema.columns
WHERE 
    table_schema = 'public'
ORDER BY 
    table_name, 
    ordinal_position;
```


## 1.5 Reset id về từ 1  bằng cách sử dụng seq

```postgres
-- 1) Tạo sequence mới đếm từ 1
CREATE SEQUENCE agenda_agent_prompting_id_seq_new START 1;

-- 2) Gán lại id tăng dần từ 1 theo thứ tự cũ
UPDATE agenda_agent_prompting
SET id = s.new_id
FROM (
    SELECT id AS old_id,
           ROW_NUMBER() OVER (ORDER BY id) AS new_id
    FROM agenda_agent_prompting
) AS s
WHERE agenda_agent_prompting.id = s.old_id;

-- 3) Đặt default của cột id dùng sequence mới
ALTER TABLE agenda_agent_prompting
ALTER COLUMN id SET DEFAULT nextval('agenda_agent_prompting_id_seq_new');

-- 4) Đồng bộ giá trị hiện tại của sequence
SELECT setval('agenda_agent_prompting_id_seq_new',
              (SELECT MAX(id) FROM agenda_agent_prompting));
```

Giờ muốn xoá: agenda_agent_prompting_id_seq đi

và rename agenda_agent_prompting_id_seq_new về agenda_agent_prompting_id_seq

đi


```sql
-- 1) Xóa sequence cũ
DROP SEQUENCE IF EXISTS agenda_agent_prompting_id_seq;

-- 2) Rename sequence mới về tên cũ
ALTER SEQUENCE agenda_agent_prompting_id_seq_new 
RENAME TO agenda_agent_prompting_id_seq;
```



```sql
-- kiểm tra xem sequence
SELECT sequencename FROM pg_sequences WHERE sequencename LIKE 'agenda_agent%';

-- Liên kết sequence với cột id
ALTER SEQUENCE agenda_agent_prompting_id_seq 
OWNED BY agenda_agent_prompting.id;

-- Sau khi chạy xong, kiểm tra lại bằng:
SELECT pg_get_serial_sequence('agenda_agent_prompting', 'id');
```


---
# 2. cron job update cột nào đó vào 12h đêm 

```sql
-- Cập nhật toàn bộ

UPDATE studio_bot_workflow

SET provider_name = 'groq',

    generation_params = '{"model": "openai/gpt-oss-20b", "top_p": 1, "stream": false, "temperature": 0, "reasoning_effort": "low", "max_completion_tokens": 1024}';


-- [2025-12-11 18:47:33] Connected to robot_ai_workflow
-- [2025-12-11 18:47:33] Unsafe query: 'Update' statement without 'where' updates all table rows at once


-- Chỉ cập nhật khi khác giá trị (tránh ghi đè không cần thiết)

UPDATE studio_bot_workflow

SET provider_name = 'groq',

    generation_params = '{"model": "openai/gpt-oss-20b", "top_p": 1, "stream": false, "temperature": 0, "reasoning_effort": "low", "max_completion_tokens": 1024}'

WHERE provider_name <> 'groq'

   OR generation_params IS DISTINCT FROM '{"model": "openai/gpt-oss-20b", "top_p": 1, "stream": false, "temperature": 0, "reasoning_effort": "low", "max_completion_tokens": 1024}';
```



